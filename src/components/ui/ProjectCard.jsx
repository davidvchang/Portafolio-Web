import React from 'react'

function ProjectCard({image, nameProject, title, description, children, linkGitHub, linkVisit}) {
  return (
    <div className='ProjectCard'>
        <div className='PhotoPortada'>
            <img src={image} alt={`Portada de ${nameProject}`} />
        </div>

        <div className='InfoProject'>
            <div className='titleDescription'>
                <span className='titleNameProject'>{title}</span>
                <p>{description}</p>

                <div className='usedTechnology'>
                    {children}
                </div>
            </div>

            <div className='GitHubVisit'>
                <a href={linkGitHub} target='_blank' className='btnGithub'>
                    {iconGitHub}
                    <span>Ir al repositorio</span>
                </a>

                <a href={linkVisit} target='_blank' className='btnVisit'>
                    {iconVisit}
                    <span>Ir al sitio web</span>
                </a>

            </div>
        </div>
    </div>
  )
}

const iconVisit = <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6 iconVisit ">
<path strokeLinecap="round" strokeLinejoin="round" d="M13.5 6H5.25A2.25 2.25 0 0 0 3 8.25v10.5A2.25 2.25 0 0 0 5.25 21h10.5A2.25 2.25 0 0 0 18 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25" />
</svg>

const iconGitHub = <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-github">
    <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4"/><path d="M9 18c-4.51 2-5-2-7-2"/>
</svg>

export default ProjectCard
